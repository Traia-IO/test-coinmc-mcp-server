#!/usr/bin/env python3
"""
test coinmc MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-coinmc_mcp')
...
logger.info("="*80)
logger.info(f"test coinmc MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://pro-api.coinmarketcap.com/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("test coinmc MCP Server", host="0.0.0.0")
...
# - TEST_COINMC_API_KEY: Server's internal API key for payment mode
#
...
async def community_trending_tokens(
    ...
    Example Usage:
        ...
        # With optional parameters:
        await community_trending_tokens(limit=5)
...
async def community_trending_topics(
    ...
    Example Usage:
        ...
        # With optional parameters:
        await community_trending_topics(limit=5)
...
async def listings_historical(
    ...
    Example Usage:
        # Minimal (required params only):
        await listings_historical(date="example")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-coinmc-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "test-coinmc-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting test coinmc MCP Server")
    logger.info("="*80)
