...
# Script to build and run the test coinmc MCP Server locally in Docker
...
IMAGE_NAME="test-coinmc-mcp-server"
CONTAINER_NAME="test-coinmc-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running test coinmc MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# test coinmc MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ test coinmc MCP Server is running!${NC}"
echo
