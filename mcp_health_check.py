#!/usr/bin/env python
"""
test coinmc MCP Server Health Check Script

This script properly connects to the test coinmc MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if test coinmc MCP server is healthy"""
    print(f"🔍 Checking test coinmc MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected test coinmc tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check test coinmc MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="test coinmc MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 test coinmc MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ test coinmc MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ test coinmc MCP Server health check failed!")
        return 1
